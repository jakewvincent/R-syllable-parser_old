...
# Parse stepwise by nucleus, onset, and coda.
# Identify dipththongs as a single nucleus.
syllabify <- function(input, diphthong_list, onset_list, verbosity) {
  ...
  for (word in 1:length(input)) {
    ...
    # Index each seg. by splitting word into component characters & associating
    ...
    # Get adjacent vowels (if there is more than one vowel in the word)
    # & determine if they're diphthongs
    if (length(vowels$segment) > 1) {
      ## Make a df to store each sequence of two vowels
      vowel_bigrams <- as.data.frame(matrix(ncol = 2))
      ## Set x to 1; each time the loop runs it will be increased by one
      ## so that the next bigram can be extracted
      x <- 1
      ## Get positions in linear string of original word of bigrams of
      ## extracted vowels
      for (vowel in 1:nrow(vowels)) {
        # If we didn't just parse the last vowel (if x is not equal to
        # the number of vowels in the word), ...
        if (x != nrow(vowels)) {
          # ...then get the positions of those vowels & put in vowel_bigram df
          vowel_bigrams[vowel, ] <- vowels[x:(x + 1), "position"]
          x <- x + 1
        }
      }
      ## Get the diffs btw the pos'ns in the linear str to see which vowels
      ## were adjacent to each other in the original word
      vowel_bigrams$diff <- vowel_bigrams[, 2] - vowel_bigrams[, 1]
      ## If any vowels have a position difference of 1 (were actually adjacent
      ## to each other in the original string), subset only to them
      if (1 %in% vowel_bigrams$diff) {
        ...
        class(adjacent_vowels) <- c("unicode_df",
                                    "data.frame")
        ## Extract the adjacent vowels using their positions & paste them
        ## together in a new column
        ...
        ## See if any of these bigrams match the known diphthongs & indicate
        ## in adjacent_vowels w/ TRUE or FALSE
    ...
    # Add column to vowel df that says whether the vowel is part of a diphthong
    # (TRUE) or monophthong (FALSE), ...
    # and if it is part of a diphthong, which diphthong it's a part of
    # (by which row in the diphthongs df it's in).
    for (vowel in 1:nrow(vowels)) {
      if (exists("diphthongs")) {
        if (vowels$position[vowel] %in% as.character(as.matrix(diphthongs[, 1:2]))) {
          ...
          # It needs to be; otherwise is NA, in which case an error is returned
    ...
    # We now have enough information to say which vowel segments are associated
    # w/ which syllables
    # Cycle through the vowel df and create a vector for the syllable nuclei in
    # this word
    nuclei <- NA
    x <- 1
    for (vowel in 1:nrow(vowels)) {
      ...
      if (vowels[vowel, "in.diphthong"] == FALSE) {
        ...
        # If we're not at the last vowel in vowels and the vowel and the
        # following vowel are part of a diphthong together, paste them
        # together as the next nucleus
        if (vowel != nrow(vowels)) {
          if (vowels[vowel, "which.diphthong"] > 0 & vowels[vowel, "which.diphthong"] == vowels[vowel + 1, "which.diphthong"]) {
            ...
            # Indicate syllable this vowel & the following vowel was parsed into
            char_index[vowels[vowel, "position"], ]$syllable <- paste(c("\u03C3.", x),
                                                                    collapse = "")
    ...
    # Remove vowel-related objects (IMPORTANT; this can screw up the next word
    # if more than one word is supplied to the function)
    ...
    ### Parse onsets ###
    # For each unparsed consonant in the character index, starting from
    # the end of the word, ...
    for (consonant in rev(subset(char_index, parsed == FALSE)$position)) {
      # If the consonant is preceding the first vowel, parse it into the
      # first syllable
      if (char_index[consonant, "position"] < vowels[1, "position"]) {
        ...
        # Otherwise, if the position of the unparsed consonant immediately
        # precedes a vowel, parse it with that vowel
        if ((char_index[consonant, "position"] + 1) %in% vowels$position) {
          syllabification[word, char_index[consonant + 1, "syllable"]] <- paste(c(as.character(char_index[consonant, "segment"]),
                                                                            ...
                                                                            collapse = "")
          # Indicate that it is now parsed and that it went into the syllable
          # of the vowel that follows it
          char_index[consonant, "parsed"] <- TRUE
          char_index[consonant, "syllable"] <- char_index[consonant + 1, "syllable"]
        } else {
          # Otherwise, if there are vowels following the consonant, and the
          # position of the unparsed consonant does NOT immediately precede
          # the end of the word, AND immediately precedes another consonant,
          # check if the string formed by pasting those two consonants together
          # is in the list of licit onsets.
          if (TRUE %in% (consonant < vowels$position)) {
            if (is.na(as.character(char_index[consonant + 1, "segment"])) == FALSE) {
              if (cvify(as.character(char_index[consonant + 1, "segment"])) == "C") {
                # If those two characters pasted together is in the list of
                # licit onsets, then parse the current consonant into the
                # syllable of the following consonant
    ...
    ### Parse codas ###
    # For each unparsed consonant in the character index, starting from the
    # beginning of the word, ...
    for (consonant in subset(char_index, parsed == FALSE)$position) {
      # Parse that consonant into the coda of the syllable into which the
      # preceding segment was parsed
      syllabification[word, char_index[consonant - 1, "syllable"]] <- paste(c(syllabification[word, char_index[consonant - 1, "syllable"]],
                                                                         ...
                                                                         collapse = "")
      # And mark that segment's parsed status as TRUE and indicate which
      # syllable it was parsed into
      char_index[consonant, "parsed"] <- TRUE
      char_index[consonant, "syllable"] <- char_index[consonant - 1, "syllable"]
    }
  ...
  # For each column in syllabification tbl, create collapsed version of
  # syllabified word, using periods to separate syllables
